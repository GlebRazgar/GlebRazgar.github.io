...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const titleAnimator = (function() {
    ...
    function getTitlePixels() {
      ...
      particles = (function() {
        ...
        for (let i = 0; i < initialBatch; i++) {
          ...
          // Calculate appropriate speed - simplified calculation
          const travelDistance = closestDistance; // Already calculated above
          const targetFrames = 100 + Math.floor(Math.random() * 20); // 100-120 frames (1.67-2s)
          const calculatedSpeed = (travelDistance / targetFrames) + 0.5;
        ...
        function processParticle(index) {
          ...
          const targetFrames = 100 + Math.floor(Math.random() * 20);
          const calculatedSpeed = (travelDistance / targetFrames) + 0.5;
    ...
    function drawFinalState() {
      ...
      particles.forEach(particle => {
        // Draw pixel with a thin black outline
        ctx.fillStyle = particle.color;
        const size = particle.size;
    ...
    // Draw perfect text on top for crisp edges - COMMENT OUT THIS FUNCTION
    // We don't want to draw perfect text on top as it would hide our pixel effect
    function drawPerfectText() {
      // Comment out this entire function or simply return to disable it
      return;
      
      // Original code below, but it won't execute due to the return above
      // setTimeout(() => {
      //   ...
      // }, 100);
    }
