  ...
  const titleAnimator = (function() {
    ...
    function animate() {
      ...
      if (allArrived) {
        ...
        drawFinalState();
        monitorVisibility(); // Start monitoring visibility
      } else {
        requestAnimationFrame(animate);
      }
    }
    ...
    // More aggressive redraw approach
    function monitorVisibility() {
      // Redraw on ANY scroll event
      ...
      // Redraw on ANY touch event end (for iOS)
      ...
      // Set up a regular interval to check and redraw if needed
      setInterval(function() {
        if (animationComplete && isElementInViewport(canvas)) {
          drawFinalState();
        }
    ...
    // Ensure the title is redrawn after any page resize
    window.addEventListener('resize', function() {
      if (animationComplete) {
        resizeCanvas();
        drawFinalState();
